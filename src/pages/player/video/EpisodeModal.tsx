...
const ModalComponent: React.FC<ModalComponentProps> = ({
  ...
  const [selectedEpisodeId, setSelectedEpisodeId] = useState<number | null>(defaultEpisodeId); // Track the selected episode based on ID
      ...
      <div className="bg-[#1C1C1C] w-full max-w-md h-[65vh] rounded-t-lg p-4 text-white">
        <div className="flex justify-between items-center mb-4">
          <div className="flex space-x-6 overflow-x-auto">
            {/* Episode Tab */}
            <button
              className={`pb-2 border-b-2 ${activeTab === 'episodes' ? 'border-orange-500 text-orange-500' : 'border-transparent text-gray-400'}`}
            ...
            {/* Source Tab */}
            <button
              className={`pb-2 border-b-2 ${activeTab === 'sources' ? 'border-orange-500 text-orange-500' : 'border-transparent text-gray-400'}`}
          ...
          {activeTab === 'sources' && (
            <div>
              {sources.map((source, index) => (
                <div
                  key={index}
                  className={`flex justify-between items-center bg-[#3B3B3B] p-3 rounded-lg mb-2 cursor-pointer ${index === selectedSource ? 'border border-orange-500' : ''}`}
